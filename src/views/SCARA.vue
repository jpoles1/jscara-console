...
export default Vue.extend({
	data() {
		return {
            ...
            a1_steps_per_rev: 200,
            a2_driver_teeth: 20,
            a2_receiver_teeth: 36,
            a2_steps_per_rev: 200,
            max_seg_length: 0.5,
		};
	},
	methods: {
                    ...
                    if (next_pos.X === current_pos.X && next_pos.Y === current_pos.Y && next_pos.Z === current_pos.Z) {
                        ...
                        if (rapid) {
                            ...
                            cmd_list = cmd_list.concat(scara_pos_segs.map((scara_pos) => {
                                return `G0 X${scara_pos.a1} Y${scara_pos.a2} Z${scara_pos.Z} F${this.feed_rate} (seg move)`
                            }));
                            return [cmd_list, next_pos];
                        } 
                    }
        ...
        convert_cartesian_to_scara(data: any): any {
            let gcode_lines = data.split("\n");
            ...
            this.download_gcode("scara_converted.gcode", converted_gcode.join("\n"));
        },
        load_gcode_file(file: File) {
            const reader = new FileReader();
            reader.onload = (ev) => this.convert_cartesian_to_scara(ev.target!.result);
